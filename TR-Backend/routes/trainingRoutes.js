...
// Все ручки ниже обслуживают три типа тренировок. requireEventPermission смотрит
// на eventType: team_training проверяется по команде, club_training — по клубу,
// community_training (тренировка) — по сообществу.
//
// Отметки тренировки живут не здесь, а в /api/community-events: у них своя механика
// резервной очереди, которой нет ни у команд, ни у клубов.
...
router.get('/:eventId/attendance', verifyToken, requireEventPermission('INTERNAL_VIEW', 'INTERNAL_VIEW', 'COMMUNITY_INTERNAL_VIEW'), getTrainingAttendance);

// Получить доступный состав для шторки добавления участников
// team_training → состав команды, club_training → общая база клуба
router.get('/:eventId/roster', verifyToken, requireEventPermission('INTERNAL_VIEW', 'INTERNAL_VIEW', 'COMMUNITY_INTERNAL_VIEW'), getTrainingRoster);

// Переключить статус присутствия на тренировке (self-attendance или управление руководителем)
router.post('/:eventId/attendance', verifyToken, requireEventPermission('INTERNAL_VIEW', 'INTERNAL_VIEW', 'COMMUNITY_INTERNAL_VIEW'), toggleTrainingAttendance);
...
router.get('/:eventId/lines', verifyToken, requireEventPermission('INTERNAL_VIEW', 'INTERNAL_VIEW', 'COMMUNITY_INTERNAL_VIEW'), getTrainingLines);
router.post('/:eventId/lines', verifyToken, requireEventPermission('TRAINING_LINES_MANAGE', 'CLUB_TRAINING_LINES_MANAGE', 'COMMUNITY_LINES_MANAGE'), saveTrainingLines);
...
router.get('/:eventId/plan', verifyToken, requireEventPermission('INTERNAL_VIEW', 'INTERNAL_VIEW', 'COMMUNITY_INTERNAL_VIEW'), getTrainingPlan);
router.post('/:eventId/plan', verifyToken, requireEventPermission('TRAINING_PLAN_MANAGE', 'CLUB_TRAINING_PLAN_MANAGE', 'COMMUNITY_TRAINING_PLAN_MANAGE'), saveTrainingPlan);
router.put('/:eventId/plan/published', verifyToken, requireEventPermission('TRAINING_PLAN_MANAGE', 'CLUB_TRAINING_PLAN_MANAGE', 'COMMUNITY_TRAINING_PLAN_MANAGE'), setPlanPublished);
...
router.post('/:eventId/plan/adhoc', verifyToken, requireEventPermission('TRAINING_PLAN_MANAGE', 'CLUB_TRAINING_PLAN_MANAGE', 'COMMUNITY_TRAINING_PLAN_MANAGE'), createAdhocDrill);
router.put('/:eventId/plan/:itemId/adhoc', verifyToken, requireEventPermission('TRAINING_PLAN_MANAGE', 'CLUB_TRAINING_PLAN_MANAGE', 'COMMUNITY_TRAINING_PLAN_MANAGE'), updateAdhocDrill);
