...
// ─── Основной компонент ───────────────────────────────────────────────────
export function TournamentCardGame({ game, onGameClick }) {
  const isFinished = game.status === 'finished';
  const isLive = game.status === 'live';
  const hasTime = !!game.game_date;

  const isPlayedOrLive = isFinished || isLive;
  const canOpen = isPlayedOrLive;
  ...
  return (
    <div
      className={clsx(
        'w-full bg-surface-base rounded-3xl border px-2 pb-2 pt-4 flex flex-col shadow-md gap-2 relative overflow-hidden select-none transition-all',
    ...
    >
      ...
      <div className="w-full grid grid-cols-[1fr,auto,1fr] items-center text-[10px] font-bold text-content-muted border-t border-surface-level2 pt-3 px-0.5 relative">
        ...
        <div className="flex justify-end shrink-0 opacity-60 pr-1">
          {game.game_number && (
