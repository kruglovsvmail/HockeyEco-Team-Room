...
export const MeetingAttendance = ({
  ...
  const hasAttendanceSubscription = checkAccess('MEETING_ATTENDANCE_MANAGE', event?.my_team_id);
  ...
  const isAttendanceManagerRole = useMemo(() => {
    ...
    const allowedLow = (PERMISSIONS.MEETING_ATTENDANCE_MANAGE?.allowedRoles || [])
  ...
  // ── Список доступных для добавления ──────────────────────────────────────
  ...
  const handleMarkUser = async (playerObj) => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/api/meetings/${event.event_id}/attendance`, {
        method:  'POST',
  ...
  const handleTogglePayTag = async (e, attendeeUser) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || '';
      await fetch(`${apiUrl}/api/meetings/${event.event_id}/attendance-tag`, {
        method:  'PUT',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body:    JSON.stringify({
          eventType:    event.event_type,
      ...
      <div className="flex flex-col gap-2 w-full" onClick={handleContainerClick}>
        ...
        <BottomSheet isOpen={isSheetOpen} onClose={() => setIsSheetOpen(false)}>
          <div className="flex flex-col gap-4">
            <h3 className="text-[18px] font-black text-content-main mb-2">Отметить участника</h3>
