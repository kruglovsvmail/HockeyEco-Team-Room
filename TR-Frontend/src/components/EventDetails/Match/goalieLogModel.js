...
// Дефолт — «не указан», а НЕ «пустые ворота»: если до момента t сторону вообще
// не заполняли, мы про вратаря ничего не знаем. Пустые ворота — это осознанное
// действие (сняли вратаря), и оно всегда задаётся явной точкой таймлайна.
// Разница принципиальна для статистики: гол в пустые ворота добавляется к броскам
// соперника, а гол «неизвестному» вратарю — нет (он уже внутри введённых бросков).
function sampleAt(timeline, t) {
  let val = { goalie_id: null, unspecified: true };
