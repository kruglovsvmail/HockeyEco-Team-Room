...
export const MatchAttendance = ({ event, initialAttendees = [], initialTeamRoster = [], initialStaffMembers = [], initialDraftLines = [], refreshData, openRightPanel }) => {
  ...
  const [isDeletingUser, setIsDeletingUser] = useState(false); // спиннер кнопки удаления в шторке 
  ...
  const hasAttendanceSubscription = checkAccess('MATCH_ATTENDANCE_MANAGE', event?.my_team_id);
  ...
  const isAttendanceManagerRole = useMemo(() => {
    ...
    const allowedRoles = PERMISSIONS.MATCH_ATTENDANCE_MANAGE?.allowedRoles || 
                         (Array.isArray(PERMISSIONS.MATCH_ATTENDANCE_MANAGE) ? PERMISSIONS.MATCH_ATTENDANCE_MANAGE : []);
  ...
  // Снявшиеся после дедлайна (withdrawn_at) уходят из основных списков в свой
  // блок: играть они не будут, но взнос за матч за ними остаётся.
  ...
  const goalies = useMemo(() => {
    return attendees.filter(a => !isWithdrawn(a) && (a.position === 'goalie' || a.position === 'G'));
  }, [attendees]);
  ...
  const handleMarkUser = async (playerObj) => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/api/matches/${event.event_id}/attendance`, {
        method: 'POST',
  ...
  const handleTogglePayTag = async (e, attendeeUser) => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || '';
      await fetch(`${apiUrl}/api/matches/${event.event_id}/attendance-tag`, {
        method: 'PUT',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          eventType: event.event_type,
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (isPlayerInLines) {
          ...
          await fetch(`${apiUrl}/api/matches/${event.event_id}/lines`, {
            method: 'POST',
            headers: { ...headers, 'Content-Type': 'application/json' },
            body: JSON.stringify({
              teamId: event.my_team_id,
            ...
            hasAttendanceSubscription ? (
              ...
              <div className="absolute -bottom-1.5 -left-1.5 z-10">
                <HintPopover status="no_subscription">
                  <button
                    type="button"
                    className="w-[22px] h-[22px] rounded-full flex items-center justify-center text-[10px] font-black shadow-md bg-surface-level3 text-content-muted opacity-30 border border-surface-border cursor-pointer"
                  >
                    ₽
                  </button>
                </HintPopover>
  ...
  return (
    <FadeIn className="flex flex-col gap-2 pb-32 min-h-[30vh]">
      <div className="flex flex-col gap-2 w-full" onClick={handleContainerClick}>
          ...
          {withdrawn.length > 0 && (
            <ContainerContent title="Снялись после деадлайна" count={withdrawn.length}>
              <div className="grid grid-cols-[repeat(auto-fill,minmax(94px,1fr))] gap-y-5 gap-x-2 justify-items-center mt-2 opacity-70">
                {withdrawn.map((attendeeUser, idx) => renderAttendeeCard(attendeeUser, idx))}
              </div>
              <div className="text-[10px] text-content-muted leading-tight mt-4 px-1">
                Отметка снята после дедлайна: в состав они не идут, но взнос за матч
                сохраняется — они остаются в расчёте стоимости.
              </div>
            </ContainerContent>
          )}
        </div>
        ...
        <BottomSheet isOpen={!!userToRemove} onClose={() => !isDeletingUser && setUserToRemove(null)}>
          <div className="flex flex-col items-center text-center gap-4 py-2">
              ...
              <p>
                {removeIsPurge ? (
                  <>
                    ...
                    перестанет числиться в расчёте стоимости — взнос за матч с него снимется.
                  </>
                ) : removeIsLate ? (
                  <>
            ...
            <div className="flex gap-3 w-full">
              <ButtonLP variant="outline" onClick={() => setUserToRemove(null)} disabled={isDeletingUser} className="flex-1">
                Отмена
              </ButtonLP>
              <ButtonLP
                variant="primary"
