  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col gap-3">
            {isLoading ? (
              ...
              <FadeIn key="opps" className="flex flex-col gap-3">
                ...
                )) : <div className="text-center py-12 text-[14px] font-bold text-content-muted bg-surface-level1 border border-surface-border rounded-3xl p-6 shadow-sm">Ни одного соперника не добавлено</div>}
              </FadeIn>
            ) : (
              <FadeIn key="tours" className="flex flex-col gap-3">
                ...
                )) : <div className="text-center py-12 text-[14px] font-bold text-content-muted bg-surface-level1 border border-surface-border rounded-3xl p-6 shadow-sm">Список внешних лиг пуст</div>}
              </FadeIn>
            )}
          </div>
        </div>
      ...
      <ConfirmSheet
        ...
        description={<>Вы уверены, что хотите безвозвратно удалить элемент <span className="font-bold text-content-main">«{deleteConfirmation.name}»</span> из единого справочника базы данных?</>}
