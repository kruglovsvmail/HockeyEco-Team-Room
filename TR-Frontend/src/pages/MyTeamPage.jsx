...
export const MyTeamPage = () => {
  const { openRightPanel, selectedTeam, user, onTeamUpdated } = useOutletContext();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div ref={stickyHeaderRef} className="snap-start sticky top-0 z-40 shrink-0 transition-all duration-300 ease-in-out border-b border-surface-level2 bg-transparent px-4 pt-2 pb-2">
          <SegmentedControl 
        ...
        <div className="w-full overflow-hidden p-4 min-h-screen pb-[30vh]">
          {isLoading ? (
            <PageLoader />
          ) : (
        ...
        <BottomSheet isOpen={isMemberSheetOpen} onClose={() => setIsMemberSheetOpen(false)}>
          <div className="flex min-h-[200px] flex-col gap-4">
            ...
            {isSearching && <div className="text-xs font-black uppercase tracking-widest animate-pulse py-4 text-center" style={{ color: activeBrandColor }}>Поиск в базе...</div>}
            {searchResult && !searchResult.success && <div className="p-4 bg-danger-muted text-danger rounded-2xl text-xs font-normal tracking-wider text-center mt-2">{searchResult.message}</div>}
          ...
          {rosterStep === 'list' ? (
            ...
            <div className="flex flex-col gap-5 animate-fade-in text-left">
              ...
              <div className="p-3 bg-surface-level2 rounded-xl flex items-center gap-3">
                ...
                <div className="flex flex-col"><span className="text-sm font-bold text-content-main">{selectedMemberForRoster?.last_name} {selectedMemberForRoster?.first_name}</span></div>
              </div>
        ...
        <BottomSheet isOpen={!!memberToRemove} onClose={() => setMemberToRemove(null)}>
          <div className="flex flex-col items-center text-center gap-4 py-2">
            ...
            <h3 className="text-lg font-black text-content-main leading-tight">{activeTab === 'roster' ? 'Исключить из ростера?' : 'Исключить из состава?'}</h3>
