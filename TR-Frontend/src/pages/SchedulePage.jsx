...
export function SchedulePage() {
  ...
  useEffect(() => {
    const handleEventsUpdated = (e) => {
      // Сигнал шлют и тогда, когда карточку открытого события просто перечитали
      // при входе в него: данные календаря при этом не менялись, и гонять месяц
      // незачем. Отсутствие detail — старые отправители, им перечитывание нужно.
      if (e?.detail?.calendarChanged === false) return;
      fetchEvents();
    };
  ...
  const handleToggleAttendance = async (eventId, eventType, newValue, teamId, clubId = null, communityId = null) => {
    ...
    try {
      ...
      // Если человек успел открыть событие сразу после тумблера, его детали
      // держат карточку, отданную календарём до отметки. Перечитать её при
      // открытии мало: тот запрос мог уйти раньше, чем сюда пришёл ответ на POST,
      // и вернуть всё ту же старую долю. Просим карточку ещё раз — теперь уже
      // заведомо после того, как отметка легла в базу. Календарь мы только что
      // перечитали сами, поэтому calendarChanged не ставим. Когда события не
      // открыто, слушателя нет и сигнал никому не стоит ничего.
