...
import { DEADLINES, RESULTS_EDIT_WINDOW_HOURS } from '../utils/permissions';
...
export function HintPopover({ status, customContent, children, className, style }) {
  ...
  const messages = useMemo(() => ({
    ...
    // Заявка после начала неофициального матча (правило — TR-Backend/utils/lateRoster.js)
    late_roster_team_records: 'Заявку уже не изменить: в протоколе есть записи с игроками вашей команды — голы, передачи, штрафы, +/-, смены вратарей или броски. Уберите их во вкладке «Ход матча», и заявка снова откроется. Иначе статистика игроков разойдётся с протоколом.',
    late_roster_team_records_opponent: 'Заявку уже не изменить: в протоколе есть записи с игроками вашей команды. Убрать их может команда, создавшая матч. Иначе статистика игроков разойдётся с протоколом.',
    late_roster_window: `Заявку можно поменять, только пока открыт ввод результатов: ${RESULTS_EDIT_WINDOW_HOURS} ч после начала матча.`,
    late_roster_no_rights: 'После начала матча заявку меняют те, кто вносит результаты: владелец команды или клуба, менеджер, администратор.',
    late_roster_status: 'Матч отменён или вызов не подтверждён — заявка на него не подаётся.',
    ...
    not_in_community: 'Вы не состоите в этом сообществе. Вступите, чтобы отмечаться на его события.',
    not_in_group: 'Эта тренировка для другой группы. Группу назначает штаб сообщества.',
    not_team_member: 'Вы не состоите в этой команде, поэтому отметиться нельзя.',
